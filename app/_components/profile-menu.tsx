...
const hizmetAlanMenuItems: MenuItem[] = [
  ...
  { label: "Panel Özeti", icon: LayoutDashboard },
  { label: "Hizmet Taleplerim", icon: ClipboardList },
  ...
  if (layout === "mobile") {
    return (
      <div className="mt-2 flex flex-col gap-1 border-t border-border pt-3">
        ...
        {items.map((item) => (
          <MenuRow key={item.label} item={item} onNavigate={() => {}} />
        ))}
      ...
      {open && (
        ...
        >
          ...
          <div className="flex flex-col gap-0.5 py-1">
            {items.map((item) => (
              <MenuRow key={item.label} item={item} onNavigate={() => setOpen(false)} />
            ))}
          </div>

...
export function ProviderDocumentCard({
  fileName,
  fileSize,
  extension,
  previewUrl,
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onDelete}
        aria-label={`${fileName} belgesini sil`}
      ...
      <div className="flex aspect-square w-full items-center justify-center bg-background">
        ...
        {status === "error" && <AlertTriangle className="h-8 w-8 text-danger" aria-hidden="true" />}
        {status === "ready" &&
          (previewUrl ? (
            ...
            <div className="flex flex-col items-center gap-2 text-muted-foreground">
              ...
              <span className="text-xs font-semibold uppercase">{extension}</span>
            </div>
          ))}
      </div>
      ...
      <div className="flex flex-col gap-1 p-3">
        ...
        {status === "error" ? (
          <p className="text-xs text-danger">{errorMessage ?? "Belge işlenemedi."}</p>
        ) : status === "validating" ? (

// PRODUCTION dogrulamasi: https://malsevk-2.vercel.app uzerinde mobil menu +
// fotograf galerisi degisikliklerini dogrular. Prod'da DEV_ACCOUNTS seed
// edilmedigi icin (NODE_ENV==="development" gated) gercek kayit formuyla
// TAZE hesaplar olusturulur - bu hesaplar yalnizca ilgili Playwright
// context'inin localStorage'inda var olur, paylasilan bir kayit OLUSTURMAZ.
...
const REQUESTER = { name: "Prod Galeri Requester", email: `prod-galeri-req-${STAMP}@test.com`, phone: "0532 111 22 33", password: "Requester1!", role: "hizmet-alan" };
const PROVIDER = { name: "Prod Galeri Provider", email: `prod-galeri-prov-${STAMP}@test.com`, phone: "0533 444 55 66", password: "Provider1!", role: "hizmet-veren" };
...
async function login(page, account, redirect = "/panel") {
  ...
  await typeInto(page, page.locator('input[type="email"]'), account.email);
  await typeInto(page, page.locator('input[type="password"]'), account.password);
...
async function fillBaseFormFields(page, titleSuffix) {
  ...
  await page.getByLabel("İlan Başlığı").fill(`Prod Galeri Testi ${titleSuffix}`);
  await page
    .getByLabel("İş Açıklaması")
    .fill("Bu test ilanı production doğrulaması içindir ve en az yirmi karakter içerir.");
  ...
  await page.getByLabel("Operasyon Detayları").fill("Otomatik production testi için operasyon detayları.");
}
...
async function setupJobAndVerifyGallery() {
  ...
  try {
    ...
    await fillBaseFormFields(page, STAMP);
    ...
    check("ilan olusturma sirasinda konsolda JS hatasi yok (PRODUCTION)", page.jsProblems.length === 0, JSON.stringify(page.jsProblems));
    console.log(`  ilan: ${jobUrl}`);

    page.jsProblems = [];
    await verifyGalleryOnPage(page, "Hizmet Alan (ilan sahibi, PRODUCTION)");

    await registerAs(page, PROVIDER);
    await page.goto(jobUrl);
    page.jsProblems = [];
    await verifyGalleryOnPage(page, "Hizmet Veren (PRODUCTION)");
  } finally {
    await browser.close();
  }
}
...
async function verifyGalleryOnPage(page, roleLabel) {
  ...
  check(
    ...
    `dialog: ${dialogCountAfterClick}, alt ayni mi: ${altAfterMainClick === altBeforeMainClick}, url ayni mi: ${page.url() === urlBeforeMainClick}`,
  );
...
async function verifyMobileMenu(engine, engineLabel) {
  console.log(`\n[mobil menu] ${engineLabel} + mobil viewport (PRODUCTION)`);
  ...
  try {
    ...
    check("panel x=0'dan basliyor (tam genislik)", Math.abs(panelBox.x) < 2, `x: ${panelBox.x}`);
    ...
    check("kilit degeri zaman icinde SABIT kaliyor", bodyTopStillLocked === bodyTopWhileLocked, `once: "${bodyTopWhileLocked}", sonra: "${bodyTopStillLocked}"`);
...
async function verifyDesktopUnaffected() {
  console.log("\n[masaustu] menu/nav bozulmadi mi (PRODUCTION)");
  const browser = await chromium.launch();
  try {
...
async function main() {
  ...
  console.log("=== Kurulum: 6 fotografli (1 portre dahil) test ilani olusturuluyor + galeri kontrolleri (PRODUCTION) ===");
  ...
  console.log(anyFail ? "\nSONUC: EN AZ BIR KONTROL BASARISIZ (PRODUCTION)." : "\nSONUC: TUM KONTROLLER PRODUCTION'DA DA GECTI.");

...
**~~§20'deki düzeltmeleri mevcut migration dosyalarına uygulamak~~ — ✅ TAMAMLANDI.** **~~Boş, gerçek bir Supabase projesine karşı `supabase db reset` ile bir dry-run~~ — ✅ TAMAMLANDI** (bkz. "Yerel Migration Dry-Run Sonucu" bölümü) — tamamen yerel/izole bir ortamda, 4 gerçek hata bulunup düzeltildi, iki-üç ardışık `db reset` hatasız, 9/9 negatif güvenlik testi + 1 pozitif kontrol geçti. **~~Gerçek, hosted bir development projesine `supabase db push` + gerçek GoTrue/JWT/RLS/Storage akışlarıyla canlı test~~ — ✅ TAMAMLANDI**, 2 kritik çalışma-zamanı hatası bulundu ve `0022` ile düzeltildi (bkz. §25). Kalan gerçek sonraki adım artık **production/staging'e geçiş** — bu belgenin ve bu görevin kapsamı dışında, ayrı bir yetkilendirme gerektirir; ayrıca §25'in kendi "Açık kalan bulgular" alt bölümündeki `service_role` yetki boşluğu ve Auth e-posta hız sınırı/SMTP eksikliği hâlâ çözülmedi.

## 25. `0022` — Profile Bootstrap ve Offer Lifecycle Runtime Fix

> Bu bölüm, `0001`–`0021`'in gerçek, hosted bir development Supabase projesine (`trfnmpihcnriqgikglpu`, `supabase link` ile bağlı) uygulanıp gerçek GoTrue kayıt/JWT/RLS/Storage akışlarıyla canlı test edilmesi SONRASINDA (ayrı bir görev turunda, kullanıcı tarafından sağlanan canlı doğrulama raporunda) bulunan iki kritik çalışma-zamanı hatasının `supabase/migrations/0022_fix_profile_bootstrap_and_offer_lifecycle.sql` ile düzeltilmesini belgeler. Tarih: 2026-08-03 (§1-§24 ile aynı gün, üçüncü ayrı tur).

### 25.1 Kök nedenler

**Hata 1 — `profiles` bootstrap mekanizması hiç yoktu (KRİTİK).** 0001–0021'in HİÇBİRİNDE `auth.users` üzerinde bir trigger, ne de bir "kayıt tamamlama" RPC'si vardı — canlı test, gerçek/onaylanmış bir `auth.users` kaydı oluşturulduktan hemen sonra `public.profiles`'ta o kullanıcı için **0 satır** bulundu diye doğruladı. `0003_profiles_and_provider_catalog.sql`'in kendi yorumu ("a profiles row is created automatically the instant a Supabase Auth user is created") YANLIŞ ÇIKTI — belgelenen tasarım hiç uygulanmamıştı. Sonuç: `current_user_role()` her zaman `NULL` dönüyordu, bu da `create_job`/`create_offer`/`submit_rating` dahil rol kontrolü yapan HER RPC'yi bloke ediyordu; `authenticated`'in `profiles` üzerinde INSERT yetkisi de yoktu (0003, kasıtlı) — hiçbir API yoluyla bir kullanıcı kendi profil satırını hiç oluşturamıyordu.

**Hata 2 — teklif yaşam döngüsü RPC'leri `22P02` çalışma-zamanı hatası veriyordu (KRİTİK).** `accept_offer`/`reject_offer`/`start_work`/`record_agreement_failure`/`confirm_completion`/`dispute_completion`/`resolve_completion_dispute`/`submit_rating`'in hepsindeki ilk `SELECT`:

```sql
select o into v_offer from public.offers o where o.id = p_offer_id;
```

— hosted Postgres'e karşı doğrudan, uygulamadan tamamen bağımsız SQL ile izole edilip doğrulandı (`do $$ ... $$;` bloğu + `CONTEXT` çıktısı tam satırı gösterdi): çıplak tablo takma adının (`o`, `o.*` DEĞİL) aynı satır tipindeki bir `INTO` hedefine verilmesi, `v_offer`'ın alanlarının (ör. `v_offer.job_id`) SONRAKİ kullanımında `invalid input syntax for type uuid` hatasına yol açıyor — hata mesajının kendisi, tüm teklif satırının string'e çevrilmiş composite değerinin bir `uuid` sütununa atanmaya çalışıldığını gösteriyordu. Bu desen, `0015`'in kendi başlığında önceki bir CREATE-zamanı hatasının (`SQLSTATE 42601`, "record variable cannot be part of multiple-item INTO list") "GERÇEK düzeltmesi" olarak belgelenmişti ve yerel dry-run'da "doğrulandı" denmişti — ama o doğrulama yalnızca fonksiyonların hatasız OLUŞTUĞUNU (CREATE başarılı) gösterdi, ÇAĞRILDIĞINDA doğru çalıştığını değil; local dry-run'da `accept_offer`/... hiç gerçekten çağrılmamıştı. **Etki: bir teklif oluşturulabiliyordu ama kabul/red/işe başlama/tamamlama/itiraz/puanlama adımlarının HİÇBİRİ çalışmıyordu** — bu bulgular arasında en yüksek işlevsel önceliğe sahip olanıydı.

### 25.2 Düzeltilen fonksiyonlar

`select o into v_offer from public.offers o where o.id = p_offer_id;` deseninin TÜM proje çapında (`supabase/migrations/**`, `grep -rn "select [a-z_]+ into v_[a-zA-Z_]+ from public\.\w+ [a-z_]+ where"`) taranmasıyla, bu deseni taşıyan TEK dosyanın `0015` olduğu ve TEK 8 fonksiyonun bu 8'i olduğu doğrulandı — başka hiçbir fonksiyonda (0004/0005/0006/0007/0012/0014/0016/0018/0021, `%ROWTYPE` kullanımı hiç yok) aynı risk bulunmadı, bu yüzden 0022 yalnızca bu 8'ini düzeltir:

`accept_offer`, `reject_offer`, `start_work`, `record_agreement_failure`, `confirm_completion`, `dispute_completion`, `resolve_completion_dispute`, `submit_rating`.

(`withdraw_offer`/`request_completion` bu deseni HİÇ kullanmıyordu — ikisi de zaten `select * into v_offer from public.offers where id = ... and provider_id = auth.uid()` kalıbını kullanıyordu, dokunulmadı.)

### 25.3 Kullanılan doğru SELECT INTO deseni
...
Tablo takma adı tamamen kaldırıldı, `select o` yerine `select *` kullanıldı — bu, aynı dosyadaki `create_offer`/`withdraw_offer`/`request_completion`'ın (bug'dan hiç etkilenmeyen) ZATEN kanıtlanmış şekilde kullandığı kalıptır; yeni bir desen icat edilmedi. İki-adımlı yapı (v_offer'ı TEK BAŞINA dolduran SELECT, ardından `v_offer.job_id`'yi kullanan AYRI bir SELECT — 42601'i önlemek için hâlâ gerekli) KORUNDU. Her 8 fonksiyonda tek satır değişti; bu, her fonksiyonun eski/yeni gövdesi programatik olarak (Node.js ile) satır satır diff'lenerek doğrulandı — `accept_offer`/`resolve_completion_dispute` dışındaki 6 fonksiyonda tam olarak 1 satır fark, diğer ikisinde yalnızca yorum-satırı-sayısı farkından kaynaklanan bir kayma (içerik aynı, yalnızca konum kayması) bulundu, başka HİÇBİR fark yoktu.

Hiçbir hata kodu, iş kuralı, concurrency/CAS deseni değişmedi: gerçek "job başına tek settled teklif" garantisi hâlâ `offers_one_settled_per_job` partial unique index'i + `accept_offer`'ın kendi `unique_violation` yakalama bloğudur (0022 dokunmadı) — bu yüzden ek bir `FOR UPDATE` kilidi de EKLENMEDİ (mevcut CAS deseni zaten doğru/yeterli, yeni bir kilitleme davranışı eklemek görev kapsamı dışı bir davranış değişikliği olurdu).

### 25.4 Profiles bootstrap nihai tasarımı

`app/_lib/register-form-validation.ts` + `users.ts#RegisterInput` doğrudan koddan doğrulandı: gerçek kayıt formu `role`'ü (KULLANICI SEÇER: hizmet-alan/hizmet-veren) VE `companyName`/`companyType`/`province`/`district`'i (hepsi form-seviyesinde zorunlu — `"... zorunludur."` hata mesajlarıyla) TEK bir gönderimde toplar; bu alanlar `auth.users` INSERT anında (GoTrue'nun kendi signup çağrısında, yalnızca email/password) HİÇ mevcut değildir. Bu yüzden **hibrit tasarım** seçildi (görev talimatının kendi önerdiği yaklaşım, kod incelemesiyle doğrulanarak):

1. **`handle_new_auth_user()`** — `auth.users AFTER INSERT` trigger fonksiyonu, SADECE minimal bir `profiles` satırı açar: `id = NEW.id`, `role` NULL kalır (zaten sütunun 0003'te belgelenen nötr/varsayılan durumu). E-posta HİÇ kopyalanmaz — `profiles`'ın email sütunu yok (0003'ün kendi yorumu: "Maps StoredUser minus passwordHash/email, both owned by Supabase Auth"). `ON CONFLICT (id) DO NOTHING` ile idempotent.
2. **`complete_registration(role, full_name, phone, company_name, company_type, province, district)`** — register formunun gerçek 7 alanını, HER ZAMAN `auth.uid()` ile kilitli kendi satırına yazan yeni bir RPC. `user_id` parametre olarak ASLA alınmaz. `role` yalnızca `hizmet-alan`/`hizmet-veren` kabul eder — `admin` İSTİSNA ile reddedilir (kaynak uygulamada da admin yalnızca dev-seed ile oluşur, hiçbir kayıt formunda seçilebilir değil). `role` zaten NULL değilse (kayıt zaten tamamlanmışsa) ikinci bir tamamlama denemesi reddedilir — kaynak uygulamada bir kullanıcının kendi rolünü kayıttan SONRA self-servis değiştirebileceği hiçbir özellik yok, bu RPC yeni bir "rol değiştir" yeteneği İCAT ETMEZ.

`provider_services`/`provider_documents`/consent tabloları için zaten ayrı, çalışan RPC'ler var (`set_provider_service_categories` — 0014, `record_provider_document_consent` — 0007, `record_legal_consent` — 0008) — `complete_registration` bunları TEKRARLAMAZ, yalnızca kaynağın `registerUser()`'ının (`provider-registration.ts#registerProviderAccount`'un DEĞİL, onun sadece `users.ts` parçasının) karşılığı olan `profiles` alanlarını üstlenir.

### 25.5 Güvenlik modeli

- `handle_new_auth_user()`: `SECURITY DEFINER` + `SET search_path = public` (auth.users INSERT'ini gerçekleştiren dahili GoTrue rolünün `public.profiles` üzerinde hiç yetkisi olmayabileceği bir ortamda — nitekim bu hosted projede `service_role`'ün bile public şemaya REST erişimi yok, bkz. §25.7 — bu SECURITY DEFINER, migration'ı uygulayan rolün (postgres) kendi tam yetkisiyle çalışmasını sağlar, bu yüzden kritik). `returns trigger` olduğu için PostgREST bunu zaten hiçbir zaman bir RPC ucu olarak gösteremez; `PUBLIC`/`anon`/`authenticated`'den `EXECUTE` yine de açıkça geri alındı (savunma-derinliği, `trg_jobs_recompute_operation_terminal_state`'in — 0004 — zaten kullandığı desen).
- `complete_registration()`: `SECURITY DEFINER` + `SET search_path = public`, yalnızca `authenticated`'e (`anon`'a DEĞİL — önce gerçek bir GoTrue oturumu gerekir) `EXECUTE`. `user_id` parametre olarak alınmaz — satır her zaman `where id = auth.uid()` ile kilitlenir.
- Admin rolü İKİ bağımsız katmanda engellenir: (1) `complete_registration()`'ın kendi `p_role not in ('hizmet-alan','hizmet-veren')` kontrolü (`ML100`), (2) kaynak uygulamanın kendisinde de admin hiçbir kayıt formunda seçilebilir bir "Hesap Türü" değil — bu RPC yalnızca kaynağın zaten var olan kısıtını SQL seviyesinde tekrar uygular, yeni bir kısıt icat etmez.
- **Bulunan ve düzeltilen ek hata**: 0022'nin ilk sürümünde `COMMENT ON TRIGGER trg_auth_users_create_profile ON auth.users IS '...'` vardı — yerel dry-run bunun gerçek bir `42501 must be owner of relation users` hatası verdiğini buldu (`auth.users`, migration rolü tarafından değil `supabase_auth_admin` tarafından sahiplenilir — `0019`'un `storage.buckets` için zaten belgelediği AYNI hata sınıfı). Kaldırıldı, açıklama düz SQL yorumu olarak korundu — `CREATE TRIGGER`'ın kendisi hiç sorun değildi, yalnız sonradan yorum eklemek gerektiriyordu.
- **Bulunan ve düzeltilen ikinci ek hata**: 0022'nin ilk sürümündeki yeni hata kodları (`MLK100`–`MLK103`) 6 karakterdi. Postgres SQLSTATE'lerin TAM 5 karakter olması gerekir — PL/pgSQL bunları geçerli bir SQLSTATE olarak tanımadı VE bilinen bir "named condition" ile de eşleştiremedi, bu yüzden hepsi `42704 unrecognized exception condition`'a düştü (kendi ML100/ML101/ML102/ML103 hata kodlarım yerine). Yerel testte doğrudan gözlemlendi (`complete_registration_admin_role_error_code: "42704"` beklenen `"MLK100"` yerine), 5 karakterli `ML100`–`ML103`'e yeniden adlandırıldı (farklı önek, `MLK` ile çakışmaz), yeniden test edildi, doğru kod döndüğü doğrulandı.

### 25.6 Yerel test (Docker + Supabase CLI)

`npx supabase start` (temiz Docker ortamı) → `npx supabase db reset --local` **iki kez ardışık, hatasız** (yukarıdaki iki ek hata bulunup düzeltildikten SONRA) — 22/22 migration (0001–0022) hatasız uygulandı, `trg_auth_users_create_profile` trigger'ı ve `handle_new_auth_user`/`complete_registration`/8 düzeltilmiş offer fonksiyonu `pg_trigger`/`pg_proc` sorgularıyla doğrudan doğrulandı.

**Auth/profile testi**: Admin API ile 3 kullanıcı (A/B/C) oluşturuldu (pre-confirmed, e-posta gönderimi tetiklenmedi) → gerçek şifre-grant login → **`profiles` satırı SIFIR manuel müdahale ile otomatik oluştu** (`rowCount: 1`, `role: null`, `onboarding_completed: false`) → `create_job` tamamlama ÖNCESİ doğru şekilde bloke edildi → `complete_registration` admin-rol denemesi `ML100` ile reddedildi → eksik zorunlu alan `ML102` ile reddedildi → gerçek tamamlama başarılı (`role: "hizmet-alan"`, `onboarding_completed: true`) → ikinci tamamlama denemesi `ML101` ile reddedildi → `create_job` tamamlama SONRASI başarılı.

**Offer lifecycle uçtan uca testi** (6 bağımsız zincir, 4. hizmet-veren test kullanıcısı D ek olarak provisyon edildi): `create_offer`→`accept_offer`→`start_work`→`request_completion`→`confirm_completion`→`submit_rating` (tam mutlu yol) · `create_offer`→`reject_offer` · `create_offer`→`accept_offer`→`record_agreement_failure` · `create_offer`→`accept_offer`→`start_work`→`request_completion`→`dispute_completion`→`resolve_completion_dispute(completed)` · sahiplik-dışı kullanıcının `accept_offer` denemesi (`MLK56` engellendi) + aynı teklifi iki kez kabul etme denemesi (`MLK68` engellendi) · iki farklı sağlayıcının aynı ilana rakip teklifleri + ikincisini kabul etme denemesi (`MLK67` engellendi, `offers_one_settled_per_job` unique index'i canlı doğrulandı). **8/8 fonksiyonun HİÇBİRİNDE `22P02` görülmedi**, tüm durum geçişleri/hata kodları beklenen değerlerle eşleşti.

### 25.7 Hosted development testi

`npx supabase db push --dry-run` → yalnızca `0022_fix_profile_bootstrap_and_offer_lifecycle.sql` "pending" olarak listelendi (0001–0021 zaten uygulanmıştı) → gerçek `npx supabase db push` → `npx supabase migration list` → **local/remote 0001–0022 birebir eşleşti**.

Gerçek hosted projede (`trfnmpihcnriqgikglpu`), Admin API ile YENİ 2 test kullanıcısı oluşturulup gerçek JWT ile giriş yapıldıktan hemen sonra: **`profiles` satırı yine SIFIR manuel müdahale ile otomatik oluştu** (`rowCount: 1`, `role: null`) — yerel testle birebir aynı sonuç, artık `service_role`/manuel SQL INSERT'e (bir önceki test turunun geçici çözümüne) hiç gerek kalmadı. `complete_registration` her iki rol için de başarılı, `admin` denemesi `ML100` ile reddedildi. `create_job`→`create_offer`→`accept_offer`→`start_work`→`request_completion`→`confirm_completion`→`submit_rating` (tam mutlu yol) VE ayrıca `reject_offer`/`record_agreement_failure`/`dispute_completion`/`resolve_completion_dispute` (4 kalan dal) — **8/8 fonksiyonun HİÇBİRİNDE `22P02` görülmedi, hepsi doğru durum geçişini döndürdü**. Admin olmayan bir kullanıcının `admin_user_list`'ten veri alamadığı (0 satır) ayrıca yeniden doğrulandı — RLS/admin izolasyonunda hiçbir regresyon yok.

### 25.8 Açık kalan bulgular (bu turda BİLEREK çözülmedi)

- **`service_role`'ün bu hosted projede `public` şemasına HİÇ REST erişimi yok** (hem yeni `sb_secret_...` anahtarı hem legacy JWT — 6+ tabloda VE basit bir SQL fonksiyonunda `403/42501`) — bir önceki canlı doğrulama turunda bulundu, bu turda da AYNEN kaldı, BİLEREK çözülmedi: (1) görev talimatı açıkça yasaklıyor ("service_role açığını bu migrationa gelişigüzel GRANT ALL ile yamama"); (2) bu iki kritik hatanın hiçbiri service_role'e bağımlı değildi — ikisi de SECURITY DEFINER fonksiyon SAHİPLİĞİYLE (migration rolü) çalışıyor; (3) asıl neden (hosted platform bootstrap farkı mı, org/proje ayarı mı) hâlâ doğrulanmadı, kör bir GRANT ALL potansiyel olarak aşırı geniş bir yetki değişikliği olurdu. `0022`'nin kendi dosya başlığının Bölüm 3'ü bunu açıkça belgeler.
- **Auth e-posta gönderim hız sınırı / özel SMTP eksikliği**: bu dev projenin varsayılan/paylaşımlı e-posta servisi çok düşük bir kotaya sahip (birkaç deneme sonrası `429 over_email_send_rate_limit`) — gerçek public `/auth/v1/signup` akışının uçtan uca (yeni kayıt → e-posta onayı → giriş) gösterilmesini bu turda da engelledi; test kullanıcıları yine Admin API (`email_confirm:true`, e-posta göndermez) ile provisyon edildi. Production'a geçişten ÖNCE özel bir SMTP sağlayıcı bağlanması gerekir — bu, migration/RLS/RPC katmanının kapsamı dışında, ayrı bir operasyonel adımdır.
- **`function_search_path_mutable` (5 fonksiyon, WARN, Security Advisor)**: bir önceki canlı doğrulama turunda bulundu — 3'ü SECURITY DEFINER bile değil (saf/immutable), 2'si trigger fonksiyonu (tüm API rollerinden EXECUTE geri alınmış) — gerçek risk yok, bu turda da dokunulmadı (0022'nin kapsamı dışı).
- **`create_job`'ın `role IS NULL` durumunda MLK50 yerine MLK10 vermesi** (bu turda YAN BULGU, 0022'nin kapsamı dışı, düzeltilmedi): `current_user_role() <> 'hizmet-alan'` ifadesi `current_user_role()` NULL olduğunda SQL'in üç-değerli mantığı gereği NULL'a (TRUE'ya değil) değerlendiği için, `create_job`'ın kendi `IF` bloğu NULL rol için hiç tetiklenmiyor — akış `INSERT`'e kadar ilerleyip orada tablo-seviyeli tetikleyicinin (`0004`, `ensure_jobs_requester_is_hizmet_alan`) kendi `MLK10` hatasına takılıyor. Sonuç DOĞRU (kullanıcı yine engelleniyor) ama hata kodu/mesajı `create_job`'ın kendi belgelediğinden (`MLK50`) farklı — kozmetik, güvenlik etkisi yok, `create_offer`/`submit_rating`'in aynı `<> 'hizmet-veren'`/`<> 'hizmet-alan'` kontrollerinde de aynı şekilde var olabilir. Görev kapsamı (yalnızca bu iki kritik hata) dışında, bilinçli olarak düzeltilmedi.

### 25.9 Temizlik

Yerel: her `db reset` veritabanını sıfırlar, ayrı bir temizlik gerekmez. Hosted: test edilen tüm ilanlar `delete_job` ile silinmeye çalışıldı — tamamlanmamış/reddedilmiş/anlaşma-sağlanamamış durumdaki ilanlar başarıyla (soft-delete) silindi; **`status='completed'` bir teklifle biten 2 ilan `delete_job`'ın kendi "aktif veya tamamlanmış teklifi olan ilan silinemez" korumasıyla (`MLK92`) BİLEREK silinemedi** — bu, bir önceki test turunda da doğrulanan, tasarım gereği bir davranıştır, görev talimatı da "kalıcı veri silme mekanizması eklenmeyecek" diyor. 5 test kullanıcısının (bu turdaki 2 yeni + önceki turdan kalan 3) `profiles` satırları da aynı gerekçeyle (jobs/offers FK referansları, contact_messages/legal_consents/provider_document_consents kalıcı denetim kayıtları) silinemedi — Admin API `DELETE /auth/v1/admin/users/{id}` hepsinde `500 23503 foreign key violation` verdi. Hiçbiri gerçek kullanıcı verisi değil, hepsi açıkça "TEST"/"silinecek" etiketli.

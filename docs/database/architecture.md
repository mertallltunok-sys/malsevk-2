...
**Status: design draft değil — gerçek bir development Supabase projesine UYGULANDI ve gerçek Auth/JWT/RLS/Storage akışlarıyla canlı test edildi.** `supabase/migrations/` (Faz 1, `0001`–`0022`) önce tamamen izole, yerel bir Docker + Supabase CLI ortamına karşı gerçek bir `supabase db reset` ile baştan sona uygulandı, idempotency için ardışık kez tekrarlandı, 9 negatif güvenlik testi + 1 pozitif kontrolle doğrulandı (bkz. [SUPABASE-MIGRATION-VALIDATION.md](SUPABASE-MIGRATION-VALIDATION.md)'in "Yerel Migration Dry-Run Sonucu" bölümü) — SONRA `supabase link` ile bağlanmış gerçek bir hosted development projesine (`supabase db push`) uygulandı ve gerçek GoTrue kayıt/JWT/RLS/Storage akışlarıyla canlı test edildi. Bu canlı test, yerel dry-run'ın YAKALAYAMADIĞI iki bağımsız, kritik çalışma-zamanı hatası buldu (`profiles` bootstrap eksikliği, teklif yaşam döngüsü RPC'lerinde `22P02` hatası) — **`0022_fix_profile_bootstrap_and_offer_lifecycle.sql`** bu ikisini düzeltir ve aynı hosted projede tekrar canlı doğrulandı; bkz. "0022 — Profile Bootstrap ve Offer Lifecycle Runtime Fix" bölümü (SUPABASE-MIGRATION-VALIDATION.md). **Production'a hâlâ hiç bağlanılmadı** — yalnızca bir development projesi (`trfnmpihcnriqgikglpu`) dokunuldu; production/staging'e geçiş hâlâ ayrı, açıkça yetkilendirilmesi gereken bir sonraki iştir. **Faz 2 (Gelişmiş Yönetim ve Abonelik)** ve **Faz 3 (Ödeme ve Finans)** taslakları bilinçli olarak `docs/database/future-migrations/` altında, otomatik taranmayan bir konumda tutulur — hiçbiri hiçbir ortama (yerel/hosted/production) hiç uygulanmadı. Bkz. [migration-strategy.md](migration-strategy.md) (veri göçü) ve `docs/database/future-migrations/MANIFEST.md` (faz devreye alma sırası).

**Tablo sayısı: 35** (34/38 değil — bkz. §8, `notification_states`'in `notifications`'a birleştirilmesi 35'i 34'e indirmişti, `0021_contact_messages.sql`'in yerel dry-run'da eklenmesi Faz 1'i 19 tabloya çıkarıp toplamı tekrar 35'e getirdi). `0022` hiçbir yeni tablo eklemez (yalnızca 8 mevcut fonksiyonu düzeltir + 1 yeni trigger + 1 yeni RPC ekler) — sayı değişmedi.
...
|---|---|---|---|
| **Faz 1 — Çekirdek Pazaryeri** | `supabase/migrations/0001`–`0022` | **Evet** (yerel VE gerçek hosted bir development projesinde çalıştırıldı, ikisinde de canlı doğrulandı — bkz. SUPABASE-MIGRATION-VALIDATION.md) | Kullanıcı hesapları (+ 0022'nin `profiles` bootstrap trigger'ı/`complete_registration()`'ı), ilan/teklif/operasyon yaşam döngüsü (+ 0022'nin runtime düzeltmesi), bildirimler, belgeler, puanlama, minimum admin, temel audit, Bize Ulaşın (`contact_messages`, 0021) |
...
- **Hosted canlı testte bulunan ve `0022` ile düzeltilen 2 kritik + 2 ek çalışma-zamanı hatası** (yerel dry-run'ın bile YAKALAYAMADIĞI, yalnızca gerçek GoTrue/JWT/RPC çağrılarıyla ortaya çıkan sınıf): (1) hiçbir `auth.users` trigger'ı/RPC'si `public.profiles` satırı açmıyordu — gerçek bir kayıt kaydı oluşturulduktan sonra `profiles` 0 satır kalıyordu, bu da `current_user_role()`'ü hep `NULL` bırakıp `create_job`/`create_offer` dahil hemen her RPC'yi bloke ediyordu; (2) `accept_offer`/`reject_offer`/`start_work`/`record_agreement_failure`/`confirm_completion`/`dispute_completion`/`resolve_completion_dispute`/`submit_rating`'in hepsindeki `select o into v_offer from public.offers o where o.id = ...` (çıplak tablo takma adı, `o.*` değil) her çağrıda gerçek bir `22P02 invalid input syntax for type uuid` hatası veriyordu — teklif kabul/red/tamamlama akışının TAMAMI fiilen çalışmıyordu; (3) `0022`'nin kendi ilk sürümünde `COMMENT ON TRIGGER ... ON auth.users` ifadesi `auth.users`'ın sahibi olmadığı için (`supabase_auth_admin`, tıpkı `storage.buckets` gibi) yerel dry-run'da gerçek bir `42501` hatası verdi — kaldırıldı; (4) `0022`'nin ilk sürümündeki yeni hata kodları (`MLK100`-`MLK103`) 6 karakterdi — Postgres SQLSTATE'lerin TAM 5 karakter olması gerektiği için PL/pgSQL bunları tanımadı ve hepsi `42704 unrecognized exception condition`'a düştü; `ML100`-`ML103`'e (5 karakter) yeniden adlandırıldı. Tam ayrıntı ve hosted doğrulama kanıtı: [SUPABASE-MIGRATION-VALIDATION.md](SUPABASE-MIGRATION-VALIDATION.md)'in "0022" bölümü.

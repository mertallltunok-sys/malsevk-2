-- =============================================================================
-- MALSEVK — migration 0057: Konteyner Depolama "Konteyner Grupları"
-- (tekrarlanabilir alan grubu)
-- =============================================================================
-- AMAÇ: Tek bir Konteyner Depolama ilanının, farklı ölçü/tip/yük durumuna
-- sahip BİRDEN FAZLA konteyner grubunu (ör. "20 adet 20 ft Standart Boş" +
-- "15 adet 40 ft High Cube Dolu" + "65 adet 45 ft High Cube Dolu") aynı
-- anda taşıyabilmesi. 0056'nın eklediği DÖRT düz skaler kolon
-- (storage_container_count/size/status/content — TEK gruplu bir tasarımdı)
-- artık yetersizdir ve BU MİGRATION İLE TAMAMEN KALDIRILIR; yerine TEK bir
-- `jsonb` sütunu (storage_container_groups, bir grup NESNE dizisi) eklenir.
--
-- VERİ MODELİ KARARI (görev talimatı: "doğrudan yeni bir tablo veya ikinci
-- veri sistemi kurma; mevcut mimari yapılandırılmış servis detayları/JSON
-- kullanıyorsa onu genişlet"): `jobs` tablosunda henüz böyle bir "çok
-- değerli, yapılandırılmış alt kayıt" sütunu yoktu (customsRequestedServices/
-- recyclingScopeOfWork gibi mevcut diziler yalnızca DÜZ STRING listeleridir,
-- NESNE dizisi değildir) — bu yüzden TEK yeni bir `jsonb` sütunu en sade/
-- minimum kapsamlı çözümdür, ikinci bir tablo/ilişki İCAT EDİLMEZ. Şekil app
-- tarafında types.ts#StorageContainerGroup ile BİREBİR aynı tutulur
-- (types.ts kendi kuralı gereği import etmediği için orada AYRICA inline
-- tanımlanır — iki taraf da elle senkron tutulmalıdır, PL/pgSQL TypeScript'i
-- içe aktaramaz). Yalnızca minimal bir "dizi mi" sağlık kontrolü eklenir
-- (TAM bir JSON-schema CHECK'i İCAT EDİLMEZ — her grubun koşullu alt
-- alanlarının kendi iç tutarlılığı zaten app tarafında, storage-container-
-- catalog.ts#sanitizeStorageContainerGroup'ta garanti edilir; bu satır
-- yalnızca sütunun HER ZAMAN bir jsonb DİZİSİ, ya da null, olmasını
-- sağlar) — 0056'nın kendi jsonb-array CHECK deseniyle AYNI, bu projenin
-- İLK (jsonb tabanlı) konteyner tasarımının kurduğu ilkeye GERİ DÖNÜŞ.
--
-- storageProductType/Quantity/Unit/Tonnage ("Depolanacak Ürün Bilgileri")
-- Konteyner Depolama'da ARTIK HİÇ KULLANILMAZ (0056'nın "mükerrer alan
-- düzeltmesi" relabel denemesi — TEK grup varken mümkündü, N grup varken
-- TEK değerli bu alanlar yetersiz kalıyordu — bu migration ile TERK
-- EDİLDİ); bu dört kolonun KENDİSİ hiç değişmez, yalnızca app kodu artık bu
-- kategoride bunları hiç yazmaz/okumaz (bkz. job-store.ts#
-- resolveStorageProductFields).
--
-- RPC GÜVENLİK DİSİPLİNİ (0032-0034/0046-0048/0051/0054/0056'nın kurduğu,
-- tekrar eden desen): `create_job`/`update_job_as_admin`/
-- `update_job_as_requester` güncel TAM imzalarıyla (0056 sonrası, doğrudan
-- pg_get_function_identity_arguments ile canlı veritabanından
-- doğrulanmıştır) önce `drop function if exists` edilir, SONRA yeni imza
-- yazılır. `create_operation_with_jobs`ın imzası YİNE DEĞİŞMEZ (5
-- parametre) — yalnızca gövde, p_services'in her elemanının içinden
-- 'storage_container_groups' okunur (0030'un per-service province
-- override'ı EKLEDİĞİ AYNI desen).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- BÖLÜM 1 — jobs.storage_container_* dört düz kolon kaldırılır, TEK jsonb sütunu eklenir
-- -----------------------------------------------------------------------------
alter table public.jobs drop constraint if exists jobs_storage_container_count_positive;
alter table public.jobs drop constraint if exists jobs_storage_container_content_requires_dolu;
alter table public.jobs drop column if exists storage_container_count;
alter table public.jobs drop column if exists storage_container_size;
alter table public.jobs drop column if exists storage_container_status;
alter table public.jobs drop column if exists storage_container_content;

alter table public.jobs add column if not exists storage_container_groups jsonb null;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'jobs_storage_container_groups_is_array'
  ) then
    alter table public.jobs add constraint jobs_storage_container_groups_is_array
      check (storage_container_groups is null or jsonb_typeof(storage_container_groups) = 'array');
  end if;
end $$;

comment on column public.jobs.storage_container_groups is
  'types.ts#Job.storageContainerGroups ile birebir — Konteyner Depolama''nın tekrarlanabilir konteyner grupları (adet/ölçü/tip/yük durumu + koşullu yük içeriği/brüt ağırlık/tehlikeli madde/UN/IMO/reefer sıcaklık-elektrik). Yalnızca storage-container-catalog.ts#isContainerStorageCategory(category) true iken app tarafından doldurulur/gösterilir — bu sütun kendi başına bir görünürlük kuralı UYGULAMAZ. Bu kategoride storage_product_type/quantity/unit/tonnage ARTIK KULLANILMAZ (her zaman null).';

-- -----------------------------------------------------------------------------
-- BÖLÜM 2 — create_job: 4 düz parametre çıkarıldı, TEK jsonb parametre eklendi (45 parametreden 42'ye)
-- -----------------------------------------------------------------------------
drop function if exists public.create_job(
  ...
  text, text, text[], text, text[], text, numeric, text, numeric, text, integer, text, text, text
);
...
begin
  ...
  if p_storage_container_groups is not null and jsonb_typeof(p_storage_container_groups) <> 'array' then
...
-- yalnızca gövde, p_services'in her elemanının içinden 'storage_container_groups' okunur.
-- -----------------------------------------------------------------------------
...
begin
  ...
  for v_service in select * from jsonb_array_elements(p_services) loop
    ...
    if v_service_container_groups is not null and jsonb_typeof(v_service_container_groups) <> 'array' then
      raise exception 'MLK55: storage_container_groups must be a jsonb array (service %)', v_index using errcode = 'MLK55';
    end if;
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 4 — update_job_as_admin: 4 düz parametre çıkarıldı, TEK jsonb parametre eklendi (40 parametreden 37'ye)
-- -----------------------------------------------------------------------------
drop function if exists public.update_job_as_admin(
  ...
  numeric, text, numeric, text, timestamp with time zone, integer, text, text, text
);
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 5 — update_job_as_requester: 4 düz parametre çıkarıldı, TEK jsonb parametre eklendi (40 parametreden 37'ye)
-- -----------------------------------------------------------------------------

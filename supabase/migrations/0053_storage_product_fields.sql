-- =============================================================================
-- MALSEVK — migration 0053: Depolama (Depo Hizmetleri) ürün bilgisi alanları
-- =============================================================================
-- AMAÇ: "Depolama grubuna bağlı ilanlarda depolanacak ürün bilgisi" görev
-- talimatı. Depo Hizmetleri grubunun (service-catalog.ts#STORAGE_SERVICE_
-- GROUP_ID = "depo-hizmetleri", 12 alt kategori — bkz. app kodu değişikliği,
-- service-catalog.ts#isStorageGroupCategory artık bu grubun TEK doğruluk
-- kaynağı) TÜM üyelerinde kullanılan 4 yeni alan: Ürün Cinsi (mevcut
-- ProductTypeCombobox/PRODUCT_TYPE_CUSTOM_VALUE deseninin AYNEN yeniden
-- kullanımı — bkz. app kodu), Miktar, Birim (kg/ton/adet —
-- recycling_unit ile AYNI 3 sabit değer, İKİNCİ bir birim kataloğu İCAT
-- EDİLMEDİ), Toplam Tonaj (isteğe bağlı).
--
-- İKİNCİ BİR İLAN/ÜRÜN VERİ MODELİ YOK: mevcut `product_quantity`/
-- `product_type`/`product_tonnage` (Liman Hizmetleri/Nakliye'ye özel, her
-- zaman "adet" birimini varsayar) BİLEREK yeniden kullanılmadı — Depolama'nın
-- "Birim" alanı esnek olmalı (kg/ton/adet), bu da Liman/Nakliye'nin sabit
-- "adet" varsayımıyla ÇAKIŞIR; bu yüzden recycling_material_category_id/vb.
-- ile AYNI "kategoriye özel, kendi kolonları" deseni izlendi — Liman/Nakliye/
-- Gümrük Müşavirliği/Geri Dönüşüm/Nakliye'nin HİÇBİR alanı/RPC'si/kolonu
-- DOKUNULMADI.
--
-- RPC GÜVENLİK DİSİPLİNİ (0032/0033/0034/0046/0047/0048'in AYNI dersi):
-- imzası değişen HER RPC'nin ("create_job", "update_job_as_admin",
-- "update_job_as_requester") mevcut TAM imzası önce `drop function if
-- exists` ile silinir, SONRA yeni parametreler eklenmiş hâli yazılır —
-- imzalar bu migration'dan HEMEN ÖNCEKİ canlı durumdan (doğrudan Postgres
-- bağlantısıyla `pg_get_function_identity_arguments` ile doğrulandı, proje
-- raporuna bkz.) alınmıştır. `create_operation_with_jobs`in imzası
-- DEĞİŞMİYOR (0046/0048 ile AYNI gerekçe — parametreler zaten jsonb, yeni
-- alanlar p_services'in içindeki her servis nesnesinin yeni anahtarlarından
-- okunuyor).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- BÖLÜM 1 — jobs.storage_product_type / storage_product_quantity /
-- storage_product_unit / storage_product_tonnage
-- -----------------------------------------------------------------------------
alter table public.jobs add column if not exists storage_product_type text;
alter table public.jobs add column if not exists storage_product_quantity numeric;
alter table public.jobs add column if not exists storage_product_unit text;
alter table public.jobs add column if not exists storage_product_tonnage numeric;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'jobs_storage_product_unit_valid'
  ) then
    alter table public.jobs add constraint jobs_storage_product_unit_valid
      check (storage_product_unit is null or storage_product_unit in ('kg', 'ton', 'adet'));
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'jobs_storage_product_quantity_positive'
  ) then
    alter table public.jobs add constraint jobs_storage_product_quantity_positive
      check (storage_product_quantity is null or storage_product_quantity > 0);
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'jobs_storage_product_tonnage_positive'
  ) then
    alter table public.jobs add constraint jobs_storage_product_tonnage_positive
      check (storage_product_tonnage is null or storage_product_tonnage > 0);
  end if;
end $$;

comment on column public.jobs.storage_product_type is
  'types.ts#Job.storageProductType ile birebir — yalnızca service-catalog.ts#isStorageGroupCategory(category_id) kapsamında dolu, diğer TÜM kategorilerde null. Katalog önerisi ya da serbest metin (product-catalog.ts#PRODUCT_TYPE_CUSTOM_VALUE deseninin aynısı, sentinel asla buraya yazılmaz — yalnızca gerçek metin).';
comment on column public.jobs.storage_product_quantity is 'types.ts#Job.storageProductQuantity — ondalıklı, 0dan büyük.';
comment on column public.jobs.storage_product_unit is 'types.ts#Job.storageProductUnit — recycling_unit ile AYNI 3 sabit değer (kg/ton/adet), ikinci bir birim kataloğu yok.';
comment on column public.jobs.storage_product_tonnage is 'types.ts#Job.storageProductTonnage — isteğe bağlı, 0dan büyük.';

-- -----------------------------------------------------------------------------
-- BÖLÜM 2 — create_job: mevcut TAM 36 parametreli imza (0048) önce silinir.
-- -----------------------------------------------------------------------------
...
comment on function public.create_job(
  ...
  '0053: p_storage_product_type/quantity/unit/tonnage eklendi (Depo Hizmetleri grubunun 4 yeni alanı) — imza 36''dan 40 parametreye çıktı, mevcut 36 parametreli overload BİLEREK drop edildi (stale-overload sınıfı hatayı önlemek için, bkz. 0032/0033/0034/0046/0048 dersi).';
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 3 — create_operation_with_jobs: imza DEĞİŞMİYOR (0046/0048 ile AYNI
-- gerekçe) — yeni alanlar p_services'in içindeki her servis nesnesinin
-- storage_product_* anahtarlarından okunuyor (yalnızca Depo Hizmetleri
-- hizmet kartında dolu, diğer kardeş hizmetlere hiç kopyalanmaz — çağıran
-- taraf, job-store.ts#resolveStorageProductFields, zaten yalnızca kapsam
-- içindeki servise bu anahtarları yazar).
-- -----------------------------------------------------------------------------
...
comment on function public.create_operation_with_jobs(text, text, jsonb, jsonb, uuid) is
  '0053: her p_services elemanı artık storage_product_type/quantity/unit/tonnage anahtarlarını da (yalnızca Depo Hizmetleri hizmet kartında dolu) okuyup ilgili jobs kolonlarına yazıyor — imza DEĞİŞMEDİ, yalnızca gövde genişledi (0048''in customs_transaction_type''ı eklerken izlediği AYNI desen).';

-- -----------------------------------------------------------------------------
-- BÖLÜM 4 — update_job_as_admin: mevcut TAM 31 parametreli imza (0048) önce
...
comment on function public.update_job_as_admin(
  ...
  '0053: 0048''in kapsamına Depo Hizmetleri''nin 4 alanı (storage_product_type/quantity/unit/tonnage) eklendi, AYNI coalesce(p_x, x) disipliniyle. Kategori (category_id)/fotoğraflar hâlâ bu RPC''nin kapsamı dışında (bkz. 0037''nin kendi gerekçesi).';
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 5 — update_job_as_requester: mevcut TAM 31 parametreli imza (0051)
...
-- korunur. Görev kapsamı admin düzenlemesini açıkça istiyor; bu RPC de
-- eklendi çünkü aksi halde ilan sahibinin admin kararından ÖNCEki kendi
-- düzenlemesi (job-edit-form.tsx) bu 4 yeni alanı sunucuya hiç yansıtamazdı
-- (bkz. 0051'in kendi "senkronizasyon" gerekçesi, AYNEN geçerli).
-- -----------------------------------------------------------------------------
...
comment on function public.update_job_as_requester(
  ...
  '0053: update_job_as_admin (0053) ile AYNI 4 yeni alan (storage_product_*) eklendi, AYNI coalesce disipliniyle — sahiplik/moderation_status=pending_review kısıtları (0051) değişmedi.';
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 6 — admin_job_list view: `j.*` kullandığı için yeni kolonları otomatik
-- kapsar, ama 0035/0048 ile AYNI nedenle (yeni kolonlar fiziksel sırada
...
comment on view public.admin_job_list is
  '0053: jobs''a storage_product_* eklenmesi nedeniyle drop+recreate edildi (0035/0048 ile AYNI gerekçe) — davranış/erişim değişmedi, yalnızca j.* artık bu 4 yeni kolonu da kapsıyor.';

-- =============================================================================
-- MALSEVK — migration 0063: Nakliye "Ölçü ve Yerleşim Bilgileri"
-- =============================================================================
-- AMAÇ: "MALSEVK Nakliye Ölçü ve Yerleşim Bilgileri" görev talimatı — Yükün
-- Hazırlanış Biçimi kartının içinde açılan, TAMAMEN isteğe bağlı sade bir
-- ölçü/yerleşim alt kartı (Palet Ölçüsü/En/Boy/Yükseklik/Çap/Hacim/Yerleşim
-- Biçimi/En Fazla İstif Katı — hangi alanların hangi hazırlanış biçiminde
-- anlamlı olduğu app/_lib/nakliye-transport-catalog.ts#getMeasurementFieldGroup'ta
-- merkezi olarak tanımlıdır). Eski, kaldırılmış zengin `NakliyePackagingEntry`
-- alt-form sisteminin YERİNE GEÇMEZ.
--
-- MİMARİ KARAR ("mevcut projedeki JSON/details desenine uygun merkezi ve tip
-- güvenli bir yapı" — görev talimatının kendi izniyle): tek tek dağınık
-- sütunlar (En/Boy/Yükseklik/Çap/vb. için ayrı ayrı) AÇILMADI — bunun yerine
-- `storage_container_groups` (0056/0057) İLE BİREBİR AYNI "yapılandırılmış/
-- değişken şekilli alt veri için TEK jsonb sütunu + minimal DB şekil kontrolü,
-- derin doğrulama uygulama katmanında (nakliye-transport-catalog.ts#
-- sanitizeNakliyeDetails)" deseni izlendi — ikinci bir mimari İCAT EDİLMEDİ.
-- Admin panelinin bu alanı GERÇEKTEN görebilmesi/düzenleyebilmesi için (admin
-- yalnızca Supabase'ten okur) gerçek bir sütun gerekiyordu — bu kanıtlanmış
-- ihtiyaç, 0062'nin aynı gerekçesiyle aynı.
--
-- RPC GÜVENLİK DİSİPLİNİ (0032-0034/0054/0057/0058/0062'nin AYNI dersi):
...
-- 0062'nin BİZZAT KENDİ dosyasındaki (bu migration'dan hemen önceki canlı
-- durum) gövdelerinden alınmıştır. `create_operation_with_jobs`in imzası
-- DEĞİŞMİYOR (storage_container_groups İLE AYNI gerekçe — yeni alan her
-- servis nesnesinin kendi `nakliye_measurement_info` anahtarından okunuyor).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- BÖLÜM 1 — jobs.nakliye_measurement_info + paylaşılan minimal şekil
-- doğrulayıcı (validate_storage_container_groups İLE AYNI ilke: yalnızca
-- "jsonb bir OBJE mi" kontrolü — derin iş kuralı doğrulaması BİLEREK burada
-- DEĞİL, app tarafındadır).
-- -----------------------------------------------------------------------------
alter table public.jobs add column if not exists nakliye_measurement_info jsonb;

comment on column public.jobs.nakliye_measurement_info is
  'app/_lib/types.ts#NakliyeDetails.measurementInfo (NakliyeMeasurementInfo) ile birebir — TEK düz jsonb obje, hangi alanların dolu olduğu app/_lib/nakliye-transport-catalog.ts#getMeasurementFieldGroup''e göre değişir. YALNIZCA Nakliye''de (isTransportationCategory) anlamlı; diğer kategorilerde her zaman null. Derin doğrulama (hangi grup hangi alanı kabul eder) sanitizeNakliyeDetails''te (app kodu) yapılır — bu sütunun kendisi yalnızca "geçerli bir jsonb OBJE mi" kontrolünden geçer (aşağıdaki validate_nakliye_measurement_info).';

create or replace function public.validate_nakliye_measurement_info(p_info jsonb)
returns void
language plpgsql
...
begin
  ...
  if jsonb_typeof(p_info) <> 'object' then
    raise exception 'MLK57: nakliye_measurement_info must be a jsonb object' using errcode = 'MLK57';
  end if;
end;
$function$;

comment on function public.validate_nakliye_measurement_info(jsonb) is
  '0063: create_job/create_operation_with_jobs/update_job_as_admin/update_job_as_requester tarafından paylaşılan TEK minimal şekil kontrolü (validate_storage_container_groups İLE AYNI ilke) — yalnızca jsonb bir OBJE mi diye bakar, alan bazlı doğrulama app tarafında (nakliye-transport-catalog.ts#sanitizeNakliyeDetails).';

-- -----------------------------------------------------------------------------
-- BÖLÜM 2 — create_job: mevcut TAM 46 parametreli imza (0062) önce silinir.
-- -----------------------------------------------------------------------------
...
comment on function public.create_job(
  ...
  '0063: p_nakliye_measurement_info eklendi (Nakliye Ölçü ve Yerleşim Bilgileri — TEK jsonb sütunu, storage_container_groups İLE AYNI desen) — imza 46''dan 47 parametreye çıktı, mevcut 46 parametreli overload BİLEREK drop edildi (stale-overload sınıfı hatayı önlemek için).';
...
-- groups İLE AYNI gerekçe) — yeni alan her p_services elemanının kendi
-- `nakliye_measurement_info` anahtarından okunuyor (yalnızca Nakliye hizmet
-- kartında dolu).
-- -----------------------------------------------------------------------------
...
comment on function public.create_operation_with_jobs(text, text, jsonb, jsonb, uuid) is
  '0063: her p_services elemanı artık nakliye_measurement_info anahtarını da (yalnızca Nakliye hizmet kartında dolu) okuyup jobs''a yazıyor — imza DEĞİŞMEDİ, yalnızca gövde genişledi.';

-- -----------------------------------------------------------------------------
-- BÖLÜM 4 — update_job_as_admin: mevcut TAM 41 parametreli imza (0062) önce
-- silinir, coalesce(p_x, x) disiplini AYNEN korunur. Yeni parametre en sona
-- eklenir (0062'nin izlediği "sona ekle" konvansiyonu).
-- -----------------------------------------------------------------------------
...
comment on function public.update_job_as_admin(
  ...
  '0063: 0062''nin kapsamına Nakliye''nin nakliye_measurement_info alanı eklendi, AYNI coalesce(p_x, x) disipliniyle. Kategori (category_id)/fotoğraflar hâlâ bu RPC''nin kapsamı dışında.';
...
-- -----------------------------------------------------------------------------
-- BÖLÜM 5 — update_job_as_requester: mevcut TAM 41 parametreli imza (0062)
...
comment on function public.update_job_as_requester(
  ...
  '0063: update_job_as_admin (0063) ile AYNI yeni alan eklendi, AYNI coalesce disipliniyle — sahiplik/moderation_status=pending_review kısıtları (0051) değişmedi.';
...
-- kapsar, ama 0035/0048/0053/0054/0062 ile AYNI nedenle drop+create gerekiyor.
-- -----------------------------------------------------------------------------
...
comment on view public.admin_job_list is
  '0063: jobs''a nakliye_measurement_info eklenmesi nedeniyle drop+recreate edildi — davranış/erişim değişmedi, yalnızca j.* artık bu yeni kolonu da kapsıyor.';
